		...
		switch (i.a) {
			...
			case 'n':
				if (settings[7]) {
					...
					const datavalue = data.map((element) => {
						return element;
					});
					var id = datavalue.length + 1;
					var sound = new Audio('assets/audio/click.mp3');
					sound.volume = 0.2;
					sound.play();
				...
				break;
			case 'a':
				const dataAn = Array.from(
					document.querySelector('.announce__container').children,
				);
				const datavalueAn = dataAn.map((element) => {
					return element;
				});
...
function changeSpeed(percent) {
	...
	console.log(offsetSpeed);
	if (offsetSpeed > 245.31) {
		$('#speed-prog')
			.css('stroke-dashoffset', offsetSpeed)
			.css('stroke', 'var(--main-color)');
		if (offsetSpeed > 628.31)
			$('#speed-prog').css('stroke-dashoffset', 628.31);
	} else
		$('#speed-prog').css('stroke-dashoffset', 245.31).css('stroke', 'red');
}

function changeFuel(percent) {
	offsetFuel = 500.31 + (249 - (percent / 100) * 249);
	if (offsetFuel > 500.31) {
		document.querySelector('#fuel-prog').style.strokeDashoffset =
			offsetFuel;
		if (offsetFuel > 750.31)
			document.querySelector(
				'#fuel-prog',
			).style.strokeDashoffset = 750.31;
	} else document.querySelector('#fuel-prog').style.strokeDashoffset = 500.31;
}

const setClasses = (int) => {
	for (let i = 1; i <= 20; i++) {
		$(`#rpm_${i}`).removeClass('active');
	}
	for (let v = 1; v <= int; v++) {
		$(`#rpm_${v}`).addClass('active');
	}
};
function changeRPM(rpm) {
	if (rpm > 0.1 && rpm < 5) {
		setClasses(1);
	} else if (rpm > 5 && rpm < 10) {
		setClasses(2);
	} else if (rpm > 10 && rpm < 15) {
		setClasses(3);
	} else if (rpm > 15 && rpm < 20) {
		setClasses(4);
	} else if (rpm > 20 && rpm < 25) {
		setClasses(5);
	} else if (rpm > 25 && rpm < 30) {
		setClasses(6);
	} else if (rpm > 30 && rpm < 35) {
		setClasses(7);
	} else if (rpm > 35 && rpm < 40) {
		setClasses(8);
	} else if (rpm > 40 && rpm < 45) {
		setClasses(9);
	} else if (rpm > 45 && rpm < 50) {
		setClasses(10);
	} else if (rpm > 50 && rpm < 55) {
		setClasses(11);
	} else if (rpm > 55 && rpm < 60) {
		setClasses(12);
	} else if (rpm > 60 && rpm < 65) {
		setClasses(13);
	} else if (rpm > 65 && rpm < 70) {
		setClasses(14);
	} else if (rpm > 70 && rpm < 75) {
		setClasses(15);
	} else if (rpm > 75 && rpm < 80) {
		setClasses(16);
	} else if (rpm > 80 && rpm < 85) {
		setClasses(17);
	} else if (rpm > 85 && rpm < 90) {
		setClasses(18);
	} else if (rpm > 90 && rpm < 95) {
		setClasses(19);
	} else if (rpm > 95) {
		setClasses(20);
	} else if (rpm < 0.1) setClasses(0);
...
function loopTime() {
	var d = new Date();
	$('#upper .date #time').text(
		`${addZero(d.getHours())}:${addZero(d.getMinutes())}`,
	);
	$('#upper .date #date').text(
		`${addZero(d.getDate())}.${addZero(d.getMonth() + 1)}.${addZero(
			d.getFullYear(),
		)}`,
	);
	setTimeout(() => {
...
function hudSetting(id) {
	...
	if ($(`#${id}`).css('display') == 'none' && settings[id])
		$(`#${id}`).fadeIn();
	else $(`#${id}`).fadeOut();
	$.post(
		`https://${GetParentResourceName()}/hudSetting`,
		JSON.stringify({
			id: parseInt(id),
			bool: settings[id],
		}),
